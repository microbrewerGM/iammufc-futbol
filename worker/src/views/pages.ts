...
<button class="secondary" type="submit">${esc(metricLabel(catalog, feasibility.nearest_alternative.metric, locale.code))} — ${esc(feasibility.nearest_alternative.season)}</button>
  ...
  //
  // NOTE: feasibility.reason is generated by the feasibility resolver and is
  // still English-only (see the honest gap noted in core/locale.ts) -- the
  // surrounding chrome is localized, the computed sentence is not, yet.
  if (
...
<p>${esc(t.checkMatrix)} <a href="${base(locale)}/">${esc(t.backToMatrix)}</a></p>
  ...
  const rows = career
    .map((r) => {
      const feas = columnFeasibility(catalog, "player", r.season);
  ...
  const headFeas = columnFeasibility(catalog, "player", current.season);
